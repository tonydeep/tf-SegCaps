...
class SegCaps(object):
  def __init__(self, sess, config, is_train):
    ...
    self.images = tf.placeholder(tf.float32, [config.batch_size, 512, 512, 1])
    self.labels = tf.placeholder(tf.float32, [config.batch_size, 512, 512, 1])
  ...
  def compute_loss(self, v_lens, recons, images, labels, mask=False):
